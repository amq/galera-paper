...
Considering that the throughput stops scaling past 32 concurrent clients in our setup, we have concluded that the latency must rise at least proportionally afterwards. But how consistent is it? Sysbench provides a 95th percentile to answer this question: Galera is free of unexpected latency deviations under an increasing load, as seen in Fig. \ref{fig:varying_threads_qsingle}. Additionally, we didn't observe any measurable oscillation when running the test for a longer period of time.
